  ...
  head: () => ({
    meta: [
      { title: "Services — AI Development, Custom Software & Automation | ALStreamTech" },
      {
        name: "description",
        content:
          "End-to-end AI, custom software, web & mobile, chatbots, automation, cloud/DevOps and data analytics services engineered for enterprise scale.",
      },
    ...
    <>
      ...
      <section id="quick-overview" className="relative mx-auto max-w-7xl px-6 py-24 scroll-mt-24">
        ...
        <SectionHeading
          ...
          title={
            <>
              Eight services. <span className="text-gradient">One Integrated Team.</span>
            </>
          }
...
function ServicesHero() {
  return (
    <PageHero
      ...
      title={
        <>
          Comprehensive AI & <span className="text-gradient">Software Development</span> Solutions
        </>
      }
...
function HeroVisual() {
  const icons = [
    { Icon: Brain, x: 50, y: 10 },
    { Icon: Cpu, x: 90, y: 40 },
    { Icon: Database, x: 80, y: 85 },
    { Icon: Cloud, x: 20, y: 85 },
    { Icon: Bot, x: 10, y: 40 },
    { Icon: Network, x: 50, y: 55 },
  ];
  return (
        ...
        <div className="mt-5">
          ...
          <div className="flex flex-wrap gap-1.5">
            {service.capabilities.slice(0, featured ? 8 : 4).map((c) => (
              <span key={c} className="rounded-full bg-white/5 px-2.5 py-1 text-[11px] text-foreground/80">
                {c}
              </span>
            ))}
          </div>
        ...
        <div className="mt-4">
          ...
          <div className="flex flex-wrap gap-1.5">
            {service.technologies.map((t) => (
              <span
                key={t}
                className="glass rounded-full px-2.5 py-1 text-[11px] font-medium text-primary transition group-hover:bg-primary/10"
              >
                {t}
              </span>
            ))}
...
function ChatViz() {
  return (
    <div className="glass space-y-2 rounded-2xl p-3">
      <div className="ml-auto max-w-[70%] rounded-2xl rounded-br-sm bg-primary/20 px-3 py-1.5 text-[11px]">
      ...
      <div className="flex items-center justify-between gap-1">
        {steps.map((s, i) => (
          <div key={s} className="flex flex-1 items-center gap-1">
            <div
              className="flex h-8 flex-1 items-center justify-center rounded-lg bg-primary/10 text-[10px] font-medium text-primary animate-pulse"
    ...
    <div className="glass rounded-2xl p-3">
      <div className="grid grid-cols-3 gap-2 text-center text-[10px]">
        {[
          ...
          { i: Database, l: "DB" },
          { i: Cloud, l: "CDN" },
          { i: LineChart, l: "Monitor" },
        ].map(({ i: I, l }, k) => (
          <div
            key={l}
            className="flex flex-col items-center gap-1 rounded-lg bg-white/5 py-2 animate-pulse"
      ...
      <div className="flex h-24 items-end gap-1.5">
        {bars.map((h, i) => (
          <div
            ...
            style={{ height: `${h}%`, animation: `fade-up 0.8s ease-out both`, animationDelay: `${i * 100}ms` }}
          />
        ))}
      </div>
    </div>
    ...
    <div className="glass rounded-2xl p-3">
      <div className="grid grid-cols-4 gap-1.5">
        {Array.from({ length: 8 }).map((_, i) => (
          <div
            key={i}
            className="flex h-10 items-center justify-center rounded-lg bg-primary/10 animate-pulse"
  ...
  return (
    <section id="key-benefits" className="mx-auto max-w-7xl px-6 py-24 scroll-mt-24">
      <SectionHeading
        ...
        title={
          <>
            End-to-end <span className="text-gradient">delivery pillars</span>
          </>
        }
      />
...
function IndustriesSection() {
  return (
    <section id="industry-applications" className="mx-auto max-w-7xl px-6 py-24 scroll-mt-24">
      <SectionHeading
        eyebrow="Industries"
        title={
          <>
            Solutions Tailored for <span className="text-gradient">Every Industry</span>
          </>
        }
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {industries.map(({ icon: Icon, name, items }, i) => (
          <div
            key={name}
            className="group relative overflow-hidden rounded-3xl p-[1px] transition hover:-translate-y-1"
            style={{ animation: "fade-up 0.6s ease-out both", animationDelay: `${i * 80}ms` }}
          >
  ...
  return (
    <section id="technologies" className="relative mx-auto max-w-7xl px-6 py-24 scroll-mt-24">
      <SectionHeading
        ...
        title={
          <>
            Our <span className="text-gradient">Technology Ecosystem</span>
          </>
        }
          ...
          >
            ...
            <div className="flex flex-wrap gap-2">
              {items.map((t, k) => (
                <span
                  key={t}
                  className="glass rounded-full px-3 py-1.5 text-xs font-medium text-primary animate-float-slow transition hover:bg-primary/15 hover:shadow-glow"
      ...
      <div className="glass-strong relative overflow-hidden rounded-3xl p-10 md:p-16">
        ...
        <div className="relative mx-auto max-w-3xl text-center">
          ...
          <p className="mt-4 text-muted-foreground md:text-lg">
            Discuss your project requirements with our experts and discover how AI-powered software, automation and
            modern digital solutions can accelerate your business growth.
          </p>
          <div className="mt-8 flex flex-wrap justify-center gap-3">
            ...
            <Link
              to="/contact"
              className="glass-strong inline-flex items-center gap-2 rounded-full px-6 py-3 text-sm font-semibold hover:bg-white/10"
            >
              Get a Free Quote
            </Link>
            <Link
