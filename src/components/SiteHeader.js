      ...
      <div className="bg-transparent">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          ...
          <nav className="hidden items-center gap-1 lg:flex">
            {nav.map((item) => (<div key={item.to} className="group relative">
                <Link to={item.to} className="flex items-center gap-1 rounded-md px-3 py-2 text-sm text-foreground/80 transition hover:bg-white/5 hover:text-primary" activeProps={{ className: "text-primary" }} activeOptions={{ exact: item.to === "/" }}>
                  {item.label}
                  {item.dropdown && <ChevronDown className="h-3.5 w-3.5"/>}
                </Link>
                {item.dropdown && (<div className="invisible absolute left-0 top-full mt-1 w-64 rounded-xl border border-white/10 bg-popover/95 p-2 opacity-0 shadow-elegant backdrop-blur-xl transition-all group-hover:visible group-hover:opacity-100">
                    {item.dropdown.map((d) => (<a key={d.to} href={d.to} className="block rounded-md px-3 py-2 text-sm text-foreground/80 transition hover:bg-primary/10 hover:text-primary">
                        {d.label}
                      </a>))}
                  </div>)}
              </div>))}
          ...
          <div className="flex items-center gap-3">
            <Link to="/contact" className="hidden rounded-full bg-gradient-brand px-5 py-2.5 text-sm font-semibold text-primary-foreground shadow-glow transition hover:scale-105 md:inline-block">
              Book a Call
            </Link>
        ...
        {open && (<div className="border-t border-white/5 bg-background/95 px-6 py-4 lg:hidden">
            <nav className="flex flex-col gap-1">
              {nav.map((item) => (<Link key={item.to} to={item.to} onClick={() => setOpen(false)} className="rounded-md px-3 py-2 text-sm hover:bg-white/5 hover:text-primary">
                  {item.label}
                </Link>))}
